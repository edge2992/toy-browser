...
class socket:
    ...
    @classmethod
    def patch(cls, mocker):
        return mocker.patch("socket.socket", wraps=cls)
...
class ssl:
    ...
    @classmethod
    def patch(cls, mocker):
        return mocker.patch("ssl.create_default_context", wraps=cls)
...
class MockCanvas:
    ...
    @classmethod
    def format_paint(cls, paint, include_leading_comma=True):
        format_str = ""
    ...
    def drawCircle(self, cx, cy, radius, paint):
        format_str = (
            "drawCircle(cx={cx}, cy={cy}, radius={radius}"
    ...
    def drawString(self, text, x, y, font, paint):
        format_str = "drawString(text={text}, x={x}, y={y}" + MockCanvas.format_paint(
            paint
        )
        self.commands.append(
            (format_str + ")").format(
    ...
    def drawRRect(self, rrect, paint):
        format_str = (
            "drawRRect(bounds={bounds}, radius={radius}, "
            + MockCanvas.format_paint(paint, False)
        )
        self.commands.append(
            (format_str + ")").format(
                bounds=rrect.getBounds(),
                radius=rrect.getSimpleRadii(),
                color=paint.getColor(),
                alpha=paint.getAlpha(),
                blend_mode=paint.getBlendMode(),
            )
    ...
    def drawImage(self, image, left, top):
        self.commands.append(
            "drawImage(<image>, left={left}, top={top}".format(left=left, top=top)
        )

    def drawImageRect(self, image, src, dst):
        self.commands.append(
            "drawImageRect(<image>, src={src}, dst={dst}".format(src=src, dst=dst)
        )
    ...
    def translate(self, x, y):
        self.commands.append("translate(x={x}, y={y})".format(x=x, y=y))

    def rotate(self, degrees):
        self.commands.append("rotate(degrees={degrees})".format(degrees=degrees))
